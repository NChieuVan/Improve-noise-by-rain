...
def inference(images, is_training):
   ...
   #  layer 1
    with tf.compat.v1.variable_scope('layer_1'):
         output = tf.keras.layers.Conv2D(detail, num_feature, KernelSize, padding = 'same', kernel_initializer = initializer, 
                                   kernel_regularizer = regularizer, name='conv_1')(detail)
